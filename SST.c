...
void Chip_Erase_Operation();


unsigned char idata upper_128[128];	/* global array to store read data */
unsigned char idata security_id_32[32];	/* global array to store security_id data */
...
void Read_Cont(unsigned long Dst, unsigned long no_bytes)
{
	...
	{
		upper_128[i] = Get_Byte();	/* receive bytes */
	}
	CE_High();				/* disable device */
...
void ReadSID(unsigned char Dst, unsigned char security_length) 
{
	...
	{ 
		if (i<32)
		{
		security_id_32[i] = Get_Byte(); 
		}
		else
		{
			security_id_32[i-32] = Get_Byte(); 
		}
